...
import { PrintButton, ShiftReportDoc } from '@/lib/print';
...
export default function ShiftsPage() {
  ...
  return <div className="space-y-4">
    <PageHeader title="Cashier shifts" subtitle="Every payment is tied to an open shift; closing reconciles expected vs counted cash." actions={<div className="flex flex-wrap gap-2">{s && <PrintButton doc="shift" label="Shift report" title={`Shift ${s.number ?? ''}`} render={(ctx) => <ShiftReportDoc summary={cur} ctx={ctx} />} />}{s ? can('pos.close_shift') && <Button onClick={() => setClose(true)}><Lock />Close my shift</Button> : can('pos.open_shift') && <Button onClick={() => setOpen(true)}><LockOpen />Open shift</Button>}</div>} />
    ...
    <DataTable path="/shifts" defaultSort="opened_at" refreshKey={tick} filters={[{ key: 'status', label: 'Status', type: 'select', options: ['OPEN', 'CLOSED', 'PENDING_APPROVAL'] }, { key: 'outlet_id', label: 'Outlet', type: 'select', source: '/outlets' }, { key: 'date', label: 'Business date', type: 'date' }]}
      ...
      rowActions={(r) => <div className="flex gap-1"><PrintButton doc="shift" size="sm" variant="ghost" simple label="Report" title={`Shift ${r.number ?? ''}`} render={async (ctx) => <ShiftReportDoc summary={await get(`/shifts/${r.id}`)} ctx={ctx} />} />{r.status === 'PENDING_APPROVAL' && can('pos.approve_variance') && <Button size="sm" variant="outline" onClick={() => setApprove(r)}><ShieldCheck />Approve variance</Button>}</div>} />
