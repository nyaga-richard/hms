...
import { PrintButton, BeoDoc } from '@/lib/print';
...
export default function EventDetail() {
  ...
  return <div className="space-y-4">
    ...
    <Modal open={!!beo} onOpenChange={(o) => !o && setBeo(null)} title="Banquet Event Order" size="lg">{beo && <div className="space-y-3 text-sm">
      ...
      <div className="flex justify-end"><PrintButton doc="beo" variant="default" title={`BEO ${beo.event?.number ?? ''}`} render={(ctx) => <BeoDoc beo={beo} ctx={ctx} />} /></div></div>}</Modal>
  </div>;
}
